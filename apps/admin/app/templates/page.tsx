...
import { formatFaNumber } from '@/lib/format/number'
    ...
    <div className="grid-bg min-h-full p-5 md:p-8">
      <div className="mx-auto max-w-[1500px]">
        <PageHeader
          eyebrow="محتوا"
          title="قالب‌ها"
          description="محتوای قابل‌استفاده مجدد برای اعلان‌ها و کمپین‌ها."
          action={
            <Button
              onClick={() => {
        ...
        <div className="mb-5 grid gap-3 md:grid-cols-3">
          {loading ? (
            ...
            <>
              <Metric icon={FileText} label="قالب‌ها" value={formatFaNumber(rows.length)} />
              <Metric icon={Sparkles} label="فعال" value={formatFaNumber(rows.filter((x) => x.isActive !== false).length)} />
              <Metric icon={SlidersHorizontal} label="کانال‌ها" value={formatFaNumber(new Set(rows.map((x) => x.channel)).size)} />
            </>
          )}
        </div>
            ...
            <div className="flex flex-col gap-3 border-b p-4 lg:flex-row">
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  placeholder="جستجو بر اساس نام یا زبان…"
                  className="ps-9"
                />
              </div>
            ...
            <div>
              ...
              {!loading &&
                rows.map((x, i) => (
                  <motion.button
                    ...
                    className="grid w-full gap-2 border-b px-5 py-4 text-start transition hover:bg-muted/30 md:grid-cols-[1.6fr_.7fr_.7fr_.45fr_.5fr_auto] md:items-center"
                  >
                    <div>
                      <div className="text-sm font-semibold">{templateTitle(x)}</div>
                      <div className="mt-1 truncate text-xs text-muted-foreground">{x.subject || 'بدون موضوع'}</div>
                    </div>
                    <div>
                      <Badge variant="outline">{formatChannel(x.channel)}</Badge>
                    </div>
                    <div className="text-sm text-muted-foreground">{x.locale}</div>
                    <div className="text-sm">v{formatFaNumber(x.version ?? 1)}</div>
                    <div>
                      <Badge variant={x.isActive === false ? 'default' : 'success'}>
                        {x.isActive === false ? 'غیرفعال' : 'فعال'}
                      </Badge>
                    </div>
                    <div className="text-xs text-muted-foreground">ویرایش</div>
                  </motion.button>
                ))}
              ...
              {!loading && !rows.length && (
                <div className="p-12 text-center text-sm text-muted-foreground">
                  {result.isError
                    ? 'بارگذاری قالب‌ها ممکن نشد. احراز هویت و tenant را بررسی کنید.'
                    : 'هنوز قالبی نیست. یکی بسازید تا شروع کنید.'}
                </div>
              )}
            </div>
          </CardContent>
