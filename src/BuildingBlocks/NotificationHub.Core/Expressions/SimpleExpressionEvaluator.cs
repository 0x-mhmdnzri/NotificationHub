    ...
    private static class Tokenizer
    {
        public static List<Token> Tokenize(string input)
        {
            ...
            while (i < input.Length)
            {
                ...
                if (c == '(')
                { tokens.Add(new Token(TokenKind.LParen, "(")); i++; continue; }
                if (c == ')')
                { tokens.Add(new Token(TokenKind.RParen, ")")); i++; continue; }
                ...
                if (c is '"' or '\'')
                {
                    ...
                    while (i < input.Length && input[i] != q)
                    {
                        ...
                        len++;
                        if (len > 256)
                            throw new InvalidOperationException("String literal too long");
                    }
                    var s = input[start..i];
                    if (i < input.Length)
                        i++;
    ...
    private sealed class Parser
    {
        ...
        private bool ParsePrimary()
        {
            if (Peek().Kind == TokenKind.LParen)
            {
                Next();
                _depth++;
                if (_depth > MaxParenDepth)
                    throw new InvalidOperationException("Expression too deeply nested");
                var inner = ParseOr();
                _depth--;
            ...
            if (Peek().Kind == TokenKind.Op && Peek().Text == "exists")
            {
                ...
                if (Peek().Kind != TokenKind.Ident)
                    throw new InvalidOperationException("Expected field after exists");
                var field = Next().Text;
        ...
        private object? ReadValue()
        {
            ...
            {
                Next();
                if (double.TryParse(t.Text, NumberStyles.Any, CultureInfo.InvariantCulture, out var n))
                    return n;
                return t.Text;
            }
            if (t.Kind == TokenKind.Op && t.Text is "true" or "false")
            { Next(); return t.Text == "true"; }
            if (t.Kind == TokenKind.Ident)
            { Next(); return Resolve(t.Text); }
            throw new InvalidOperationException("Expected value");
        }

        private object? Resolve(string field)
        {
            if (!_data.TryGetValue(field, out var raw) || raw is null)
                return null;
