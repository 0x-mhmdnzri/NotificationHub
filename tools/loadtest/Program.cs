...
{
    var payload = new { recipient = channel == "sms" ? $"+98912{i:D7}" : $"user{i}@example.com", channel, templateKey = "welcome",
        data = new Dictionary<string, object?> { ["name"] = $"User{i}" }, idempotencyKey = $"st-{Guid.NewGuid():N}" };
...
static Dictionary<string, string> ParseArgs(string[] args)
{
    ...
    for (var i = 0; i < args.Length; i++) { if (!args[i].StartsWith("--")) continue; var key = args[i]; var val = i + 1 < args.Length && !args[i + 1].StartsWith("--") ? args[++i] : "true"; map[key] = val; }
    return map;
}
